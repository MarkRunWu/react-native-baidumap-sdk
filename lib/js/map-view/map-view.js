...
export default class MapView extends Component<Props> {
  ...
  pointForCoordinate(coordinate: LatLng) {
    if (Platform.OS === 'android') {
      return BaiduMapUtil.pointForCoordinate(
  ...
  coordinateForPoint(point: Point) {
    if (Platform.OS === 'android') {
      return BaiduMapUtil.coordinateForPoint(findNodeHandle(this.map), point);
    }
  }
  ...
  render() {
    ...
    return <BaiduMapView ref={ref => (this.map = ref)} {...props} />;
  }
}
